...
openai.api_key = OPENAI_KEY = os.getenv("OPENAI_GPT_API_KEY")
LATENCY = 1
    ...
    if request_url.endswith('embeddings'):
        ...
        for prompt in prompt_list:
            work_item = {
                ...
                'max_tokens': min(num_tokens_from_string(prompt), 3000),
...
def run_gpt(df, prompt_list, model="gpt-3.5-turbo", json_mode=False, timeout=60, desc=''):
    # df = df[:MAX_DATA]
    # prompt_list = prompt_list[:MAX_DATA]
    df['gpt_reuslt'], df['gpt_price'] = '',''
    ...
    for result in result_list:
        if result is None:
            ...
            total_price = price(result[1][1]['usage']["prompt_tokens"], result[1][1]['usage']["completion_tokens"], model)
            df['gpt_reuslt'][result[0]] = result[1][1]['choices'][0]['message']['content']
            df['gpt_price'][result[0]] = total_price
    return df

def get_fstring_from_prompt(prompt):
